...
module SwaggerClient
  class InlineResponse20063
    attr_accessor :id, :imdb_id, :freebase_mid, :freebase_id, :tvdb_id, :tvrage_id, :wikidata_id, :facebook_id,
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        raise ArgumentError,
              'The input argument (attributes) must be a hash in `SwaggerClient::InlineResponse20063` initialize method'
      end
      ...
      attributes = attributes.each_with_object({}) do |(k, v), h|
        unless self.class.attribute_map.key?(k.to_sym)
          raise ArgumentError,
                "`#{k}` is not a valid attribute in `SwaggerClient::InlineResponse20063`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
      ...
      self.tvdb_id = attributes[:tvdb_id] if attributes.key?(:tvdb_id)
      ...
      return unless attributes.key?(:twitter_id)
    ...
    def hash
      [id, imdb_id, freebase_mid, freebase_id, tvdb_id, tvrage_id, wikidata_id, facebook_id, instagram_id,
       twitter_id].hash
    end
